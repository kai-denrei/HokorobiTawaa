// grid.ts — mesh kernel, stages 2–5. PURE (no DOM). Ported from
// STB_BruchmalysII/src/board/grid.ts (from oskar-procedure). Determinism and
// RNG consumption order preserved. All coordinates in normalized [0,1]² space.
//
...
//
// RISK 1 (CW/CCW relaxation ordering): the closed-form `alpha` derivation orders
// corners CLOCKWISE about the centroid, but stage 4b stores CCW. The relaxer
...
function subdivide(points: Vec2[], faces: readonly (Tri | Quad)[]): {
  ...
  const vertices: Vec2[] = points.map((p) => [...p] as Vec2);
...
function seedPoints(
  ...
  if (seeder === 'hex') return hexLattice({ rings, spacing }).points;
